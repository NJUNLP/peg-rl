...
from verl.single_controller.base.decorator import Dispatch, make_nd_compute_dataproto_dispatch_fn, register, Execute
...
class COMETWorker(Worker):
    ...
    def __init__(self, config):
        ...
        print(f"forward_micro_batch_size={self.config.forward_micro_batch_size}<<<<<<<<<<<<<<<<<\n")
        self.config.forward_micro_batch_size = max(self.config.forward_micro_batch_size, 1)

        # base_tmpdir = os.environ.get("TMPDIR", tempfile.gettempdir())
        # unique_dir = os.path.join(base_tmpdir, f"tmp_rank{self.rank}_{uuid.uuid4().hex}")
        # os.makedirs(unique_dir, exist_ok=True)
        # os.environ["TMPDIR"] = unique_dir
        # print(f"TMPDIR for rank {self.rank}: {unique_dir}")
    ...
    def init_model(self):
        ...
        tokenizer_path = copy_to_local(self.config.tokenizer_path)
        self.tokenizer = hf_tokenizer(tokenizer_path)

        try:
            ckpt_path = download_model(self.config.ckpt_path)
        except Exception:
    ...
    def compute_comet_rm(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            output = DataProto.from_dict(tensors={f'{self.config.model_name}_comet_score': comet_scores}) ## need attention
    ...
    def compute_valid_comet(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            output = DataProto.from_dict(tensors={f'{self.config.model_name}_valid': comet_scores})
            output = self.ulysses_sharding_manager.postprocess_data(data=output)
        output = output.to('cpu')
...
class EmbeddingWorker(Worker):
    ...
    def __init__(self, config):
        ...
        if self.ulysses_device_mesh is not None:
            is_collect = self.ulysses_device_mesh["sp"].get_local_rank() == 0
            self._register_dispatch_collect_info(
                "embed_reward", dp_rank=self.ulysses_device_mesh["dp"].get_local_rank(), is_collect=is_collect
            )
        else:
            self._register_dispatch_collect_info("embed_reward", dp_rank=self.rank, is_collect=True)
    ...
    def init_model(self):
        # This is used to import external_lib into the huggingface systems
        from verl.workers.embedding import DataParallelEmbdding
        from torch.distributed.fsdp import CPUOffload
        ...
        if self.config.strategy == "fsdp":
            self.model = FSDP(
                ...
                sync_module_states=True,
                cpu_offload=CPUOffload(offload_params=True),
                forward_prefetch=self.config.fsdp_config.forward_prefetch,
                device_mesh=self.device_mesh,
            )
            ...
            apply_fsdp2(self.model, fsdp_kwargs, self.config.fsdp_config)
        ...
        self.embed = DataParallelEmbdding(config=self.config, model=self.model, tokenizer=self.tokenizer)
    ...
    def compute_embed_rm(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data=data)
            embed_scores = self.embed.compute_embed_rm(data=data)
            output = DataProto.from_dict(tensors={f'embed_score': embed_scores}) ## need attention
            output = self.ulysses_sharding_manager.postprocess_data(data=output)
        output = output.to('cpu')
    ...
    def compute_valid_embed(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data=data)
            embed_scores = self.embed.compute_valid_embed(data=data)
            output = DataProto.from_dict(tensors={f'embed_valid': embed_scores})
            output = self.ulysses_sharding_manager.postprocess_data(data=output)
        output = output.to('cpu')
